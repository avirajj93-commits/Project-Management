...
export const createProject = async (req, res) => {
    try {
        ...
        // check user has admin role for workshop
...
if(!workspace){
 ...
 if(!workspace.members.some((member) => member.userId === userId && member.role === 'ADMIN')){
    return res.status(403).json({ message: "You don't have admin privileges for this workspace" });
 }
...
export const addMember = async (req, res) => {
    try {
        ...
        if(project.team_lead !== userId){
            return res.status(404).json({ message: "You don't have admin privileges for this project" });
